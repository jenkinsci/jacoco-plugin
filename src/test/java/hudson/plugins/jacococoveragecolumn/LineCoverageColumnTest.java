...
public class LineCoverageColumnTest {
	private LineCoverageColumn jacocoLineCoverageColumn;

    @Before
	public void setUp() {
		jacocoLineCoverageColumn = new LineCoverageColumn();
	}
	...
	public void testGetPercentWithNoLastSuccessfulBuild() {
		...
		assertFalse(jacocoLineCoverageColumn.hasCoverage(mockJob));
		assertEquals("N/A", jacocoLineCoverageColumn.getPercent(mockJob));
		assertEquals(new BigDecimal("0.0"), jacocoLineCoverageColumn.getCoverage(mockJob));
	...
	public void testGetPercentWithBuildAndAction() {
		...
		assertTrue(jacocoLineCoverageColumn.hasCoverage(mockJob));
		assertEquals("100.0", jacocoLineCoverageColumn.getPercent(mockJob));
		assertEquals(new BigDecimal("100.0"), jacocoLineCoverageColumn.getCoverage(mockJob));
	...
	public void testGetLineColorWithNull() {
		assertNull(jacocoLineCoverageColumn.getColor(null, null));
	}
	...
	public void testGetLineColor() {
		...
		assertEquals(CoverageRange.PERFECT.getLineHexString(), jacocoLineCoverageColumn
		...
		assertEquals(CoverageRange.NA.getLineHexString(), jacocoLineCoverageColumn
	...
	public void testGetFillColorWithNull() {
		assertNull(jacocoLineCoverageColumn.getFillColor(null, null));
	}
	...
	public void testGetFillColor100() {
		...
		// without job we cannot check for NA
		assertEquals(CoverageRange.PERFECT.getFillHexString(), jacocoLineCoverageColumn
				.getFillColor(null, BigDecimal.valueOf(100)));

		// with job, we detect that it has a build, but no JaCoCoBuildAction => NA
		Job<?, ?> mockJob = new ExternalJobExtension("externaljob");
		assertEquals(CoverageRange.NA.getFillHexString(), jacocoLineCoverageColumn
	...
	public void testDescriptor() throws FormException {
		assertNotNull(jacocoLineCoverageColumn.getDescriptor());
		assertNotNull(
				jacocoLineCoverageColumn.getDescriptor().newInstance(null, JSONObject.fromObject("{\"key\":\"value\"}")));
		assertNotNull(jacocoLineCoverageColumn.getDescriptor().getDisplayName());
	}
